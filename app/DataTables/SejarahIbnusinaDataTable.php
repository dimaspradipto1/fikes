...
class SejarahIbnusinaDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<SejarahIbnuSina> $query Results from query() method.
     */
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('url_photo', function ($item) {
                if ($item->url_photo) {
                    ...
                    return '<img src="' . $url . '" alt="Foto Sejarah" class="rounded" style="width:50px;height:50px;object-fit:cover;">';
                }
            ...
            ->addColumn('action', function ($sejarahIbnuSina) {
                $btn = '<div class="d-flex justify-content-center align-items-center" style="gap: 5px;">';
                $btn .= '<a href="' . route('sejarah-ibnu-sina.edit', $sejarahIbnuSina->id) . '" class="btn btn-sm btn-warning text-white rounded shadow-sm d-flex align-items-center justify-content-center" style="width: 30px; height: 30px;" title="Edit"><i class="fa-solid fa-pen-to-square" style="font-size: 11px;"></i></a>';
                $btn .= '<form action="' . route('sejarah-ibnu-sina.destroy', $sejarahIbnuSina->id) . '" method="POST" class="m-0">' . csrf_field() . method_field('DELETE') . '<button type="submit" class="btn btn-danger btn-sm rounded shadow-sm d-flex align-items-center justify-content-center" style="width: 30px; height: 30px;" title="Hapus" onclick="return confirm(\'Yakin ingin menghapus data ini?\')"><i class="fa-solid fa-trash-can" style="font-size: 11px;"></i></button></form>';
                $btn .= '</div>';
     ...
     *
     * @return QueryBuilder<SejarahIbnuSina>
     */
    public function query(SejarahIbnuSina $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
                    ->setTableId('sejarahibnusina-table')
    ...
    {
        return 'SejarahIbnuSina_' . date('YmdHis');
    }
}
