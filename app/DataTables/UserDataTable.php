    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function ($item) {
                ...
                $badgeClass = match($item->roles) {
                    'administrator' => 'bg-danger',
                    'editor'        => 'bg-primary',
                    'author'        => 'bg-success',
                    'contributor'   => 'bg-info text-dark',
                    'subscriber'    => 'bg-secondary',
                    'customer'      => 'bg-warning text-dark',
                    'shop_manager'  => 'bg-dark',
                    'translator'    => 'bg-primary bg-opacity-75',
                    'seo_manager'   => 'bg-success bg-opacity-75',
                    'seo_editor'    => 'bg-info',
                    default         => 'bg-secondary',
                };
                $label = $roleLabels[$item->roles] ?? ucfirst($item->roles);
                return '<span class="badge rounded-pill ' . $badgeClass . '">' . $label . '</span>';
            })
            ->addColumn('action', function ($user) {
                // Hanya administrator & editor yang bisa mengelola pengguna
